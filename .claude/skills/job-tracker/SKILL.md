...
**Delete a job** — `DELETE /api/jobs/<id>`. Permanently removes it — irreversible, so
...
**Multiple changes at once** — just make several requests in sequence; there's no batch
endpoint. That's fine here since, unlike a username/password login flow, token auth doesn't
consume a rate-limited login attempt per call.
...
| `job_url` | link to the job posting, max 1000 chars. Must start with `http://` or `https://` (rejected otherwise — it's rendered as a link, so this is an XSS guard, not a formality) |
